...
class MoleculeModel(nn.Module):
    ...
    def __init__(self, args: TrainArgs):
        ...
        # when using cross entropy losses, no sigmoid or softmax during
        # training. But they are needed for mcc loss.
    ...
    def create_encoder(self, args: TrainArgs) -> None:
        ...
        if args.checkpoint_frzn is not None:
            if args.freeze_first_only:  # Freeze only the first encoder
                for param in list(self.encoder.encoder.children())[
                        0].parameters():
        ...
        if args.checkpoint_frzn is not None:
            if args.frzn_ffn_layers > 0:
                for param in list(self.ffn.parameters())[
                        0:2 * args.frzn_ffn_layers]:  # Freeze weights and bias for given number of layers
                    param.requires_grad = False
    ...
    def forward(self,
        ...
        if self.classification and not (
                self.training and self.no_training_normalization) and self.loss_function != 'dirichlet':
        ...
        if self.multiclass:
            ...
            if not (
                ...
                # training when using CrossEntropyLoss
