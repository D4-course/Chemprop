"""Message passing network (MPN) model."""
...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, hidden_size: int = None,
       ...
       """
        ...
        # layer after concatenating the descriptors if args.atom_descriptors ==
        # descriptors
    ...
    def forward(self,
        ...
        f_atoms, f_bonds, a2b, b2a, b2revb = f_atoms.to(
            self.device), f_bonds.to(
            self.device), a2b.to(
            self.device), b2a.to(
                self.device), b2revb.to(
                    self.device)
...
class MPN(nn.Module):
    ...
    def forward(self,
        ...
        if not isinstance(batch[0], BatchMolGraph):
            ...
            # TODO: handle atom_descriptors_batch with multiple molecules per
            # input
        ...
        if self.use_input_features:
            features_batch = torch.from_numpy(
                np.stack(features_batch)).float().to(
                self.device)
