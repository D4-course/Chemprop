...
def get_loss_func(args: TrainArgs) -> Callable:
    ...
    # Nested dictionary of the form {dataset_type: {loss_function:
    ...
    # Return loss function if it is represented in the
    # supported_loss_functions dictionary
...
def mcc_multiclass_loss(
    ...
    n_samples = torch.sum(
        predictions *
        masked_data_weights)  # total number of samples
...
def normal_mve(pred_values, targets):
    ...
    return torch.log(2 * np.pi * pred_var) / 2 + \
        (pred_means - targets) ** 2 / (2 * pred_var)
...
def dirichlet_class_loss(alphas, target_labels, lam=0):
    ...
    y_one_hot = torch.eye(
        num_classes, device=torch_device)[
        target_labels.long()]
...
def evidential_loss(pred_values, targets, lam=0, epsilon=1e-8):
    ...
    # Loss = L_NLL + L_REG
    # TODO If we want to optimize the dual- of the objective use the line
    # below:
