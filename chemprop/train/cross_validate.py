"""Cross validates a Chemprop model."""
...
def cross_validate(args: TrainArgs,
    ...
    if args.target_weights is not None and len(
            args.target_weights) != args.num_tasks:
        raise ValueError(
    ...
    for metric, scores in all_scores.items():
        ...
        avg_scores = multitask_mean(scores, axis=1, metric=metric)
    ...
    avg_scores = multitask_mean(
        all_scores[args.metric], metric=args.metric, axis=1)
    mean_score, std_score = np.mean(avg_scores), np.std(avg_scores)
