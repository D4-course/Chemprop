...
def molecule_fingerprint(args: FingerprintArgs,
    ...
    # only need to supply input features if using FFN latent representation
    # and if model calls for them.
    ...
    if args.fingerprint_type == 'MPN':
        # special case when we have 'descriptor' extra dimensions need to be
        # added
        if args.atom_descriptors == "descriptor":
            total_fp_size = (
                args.hidden_size + test_data.atom_descriptors_size()) * args.number_of_molecules
        else:
    ...
    all_fingerprints = np.zeros(
        (len(test_data), total_fp_size, len(
    ...
    for index, checkpoint_path in enumerate(
        ...
        if args.fingerprint_type == 'MPN' and (
                args.features_path is not None or args.features_generator):  # truncate any features from MPN fingerprint
            model_fp = np.array(model_fp)[:, :total_fp_size]
        all_fingerprints[:, :, index] = model_fp

    # Save predictions
    print(f'Saving predictions to {args.preds_path}')
    # assert len(test_data) == len(all_fingerprints) #TODO: add unit test for
    # this
    ...
    for full_index, datapoint in enumerate(full_data):
        ...
        preds = all_fingerprints[valid_index].reshape(
            (len(
                args.checkpoint_paths) * total_fp_size)) if valid_index is not None else ['Invalid SMILES'] * len(
