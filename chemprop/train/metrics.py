...
def get_metric_func(
        metric: str) -> Callable[[Union[List[int], List[float]], List[float]], float]:
    r"""
...
def bce(targets: List[int], preds: List[float]) -> float:
    ...
    """
    # Don't use logits because the sigmoid is added in all places except
    # training itself
...
def bounded_rmse(targets: List[float], preds: List[float],
                 gt_targets: List[bool] = None, lt_targets: List[bool] = None) -> float:
    """
    ...
    # replace the prediction with the target. Analogous for
...
def bounded_mse(targets: List[float], preds: List[float],
                gt_targets: List[bool] = None, lt_targets: List[bool] = None) -> float:
    """
...
def bounded_mae(targets: List[float], preds: List[float],
                gt_targets: List[bool] = None, lt_targets: List[bool] = None) -> float:
    """
...
def accuracy(targets: List[int], preds: Union[List[float],
             List[List[float]]], threshold: float = 0.5) -> float:
    """
...
def f1_metric(targets: List[int], preds: Union[List[float],
              List[List[float]]], threshold: float = 0.5) -> float:
    """
...
def mcc_metric(targets: List[int], preds: Union[List[float],
               List[List[float]]], threshold: float = 0.5) -> float:
    """
...
def sid_metric(model_spectra: List[List[float]], target_spectra: List[List[float]],
               threshold: float = None, batch_size: int = 50) -> float:
    """
    ...
    for i in trange(0, num_iters, iter_step):
        ...
        batch_mask = np.array([[x is not None for x in b]
        ...
        batch_preds[~batch_mask] = 1
        loss = batch_preds * np.log(batch_preds / batch_targets) + \
...
def wasserstein_metric(model_spectra: List[List[float]], target_spectra: List[List[float]],
                       threshold: float = None, batch_size: int = 50) -> float:
    """
