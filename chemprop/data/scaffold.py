...
def generate_scaffold(
        mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]], include_chirality: bool = False) -> str:
    """
...
def log_scaffold_stats(data: MoleculeDataset,
    ...
    index_sets = sorted(
        index_sets,
        key=lambda idx_set: len(idx_set),
        reverse=True)
    for scaffold_num, index_set in enumerate(index_sets[:num_scaffolds]):
        ...
        if logger is not None:
            ...
            for task_num, (target_avg, count) in enumerate(
                    zip(target_avgs, counts)):
                logger.debug(
