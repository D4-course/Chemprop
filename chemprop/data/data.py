...
class MoleculeDatapoint:
    ...
    def __init__(self,
                ...
                for m, reaction in zip(self.mol, self.is_reaction_list):
                    if not reaction:
                        ...
                        # for H2
                            ...
                            # not all features are equally long, so use methane
                            # as dummy molecule to determine length
                            self.features.extend(
                                np.zeros(len(features_generator(Chem.MolFromSmiles('C')))))
                    else:
                        if m[0] is not None and m[1] is not None and m[0].GetNumHeavyAtoms(
                        ) > 0:
        ...
        # Save a copy of the raw features and targets to enable different
        # scaling later on
    ...
    def extend_features(self, features: np.ndarray) -> None:
        ...
        self.features = np.append(
            self.features,
            features) if self.features is not None else features
...
class MoleculeDataset(Dataset):
    ...
    def smiles(
            self, flatten: bool = False) -> Union[List[str], List[List[str]]]:
        """
    ...
    def mols(self, flatten: bool = False) -> Union[List[Chem.Mol], List[List[Chem.Mol]],
                                                   List[Tuple[Chem.Mol, Chem.Mol]], List[List[Tuple[Chem.Mol, Chem.Mol]]]]:
        """
    ...
    def number_of_molecules(self) -> int:
        ...
        :return: The number of molecules.
        """
        return self._data[0].number_of_molecules if len(
            self._data) > 0 else None
    ...
    def features_size(self) -> int:
        ...
        """
        return len(self._data[0].features) if len(
            self._data) > 0 and self._data[0].features is not None else None
    ...
    def __getitem__(
            self, item) -> Union[MoleculeDatapoint, List[MoleculeDatapoint]]:
        r"""
...
class MoleculeSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        if self.class_balance:
            ...
            indices = [
                index for pair in zip(
                    self.positive_indices,
                    self.negative_indices) for index in pair]
        else:
...
def make_mols(smiles: List[str], reaction_list: List[bool],
              keep_h_list: List[bool], add_h_list: List[bool]):
    """
    ...
    for s, reaction, keep_h, add_h in zip(
        ...
        if reaction:
            mol.append(SMILES_TO_MOL[s] if s in SMILES_TO_MOL else (make_mol(
                s.split(">")[0], keep_h, add_h), make_mol(s.split(">")[-1], keep_h, add_h)))
        else:
            mol.append(
                SMILES_TO_MOL[s] if s in SMILES_TO_MOL else make_mol(
                    s, keep_h, add_h))
    return mol
