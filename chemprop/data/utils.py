...
def get_invalid_smiles_from_file(path: str = None,
    ...
    invalid_smiles = get_invalid_smiles_from_list(
        smiles=smiles, reaction=reaction)

    return invalid_smiles


def get_invalid_smiles_from_list(
        smiles: List[List[str]], reaction: bool = False) -> List[List[str]]:
    """
    ...
    # correspond to reaction. Therefore, get `is_mol_list` only using the
    # first element in smiles.
    ...
    # set this to False as it is not needed for invalid SMILES check
    is_adding_hs_list = [False for x in is_mol_list]
...
def get_data(path: str,
    ...
    # By default, the targets columns are all the columns except the SMILES
    # column
...
def get_inequality_targets(
        path: str, target_columns: List[str] = None) -> List[str]:
    """
...
def split_data(data: MoleculeDataset,
    ...
    if split_type == 'crossval':
        ...
        return MoleculeDataset(train), MoleculeDataset(
            val), MoleculeDataset(test)
        ...
        for i in range(len(folds)):
            if i != test_fold_index and (
                    val_fold_index is None or i != val_fold_index):
        ...
        return scaffold_split(data, sizes=sizes, balanced=True,
                              key_molecule_index=key_molecule_index, seed=seed, logger=logger)
...
def get_class_sizes(data: MoleculeDataset,
                    proportion: bool = True) -> List[List[float]]:
    """
