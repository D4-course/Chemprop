...
def build_search_space(
        search_parameters: List[str], train_epochs: int = None) -> dict:
    """
...
def load_trials(dir_path: str, previous_trials: Trials = None) -> Trials:
    ...
    hyperopt_checkpoint_files = [
        os.path.join(
            dir_path,
            path) for path in os.listdir(dir_path) if '.pkl' in path]
...
def load_manual_trials(
        manual_trials_dirs: List[str], param_keys: List[str], hyperopt_args: HyperoptArgs) -> Trials:
    """
    ...
    """
    # Non-extensive list of arguments that need to match between the manual
    # trials and the search space.
    matching_args = [
    ...
    for i, trial_dir in enumerate(manual_trials_dirs):
        ...
        with open(os.path.join(trial_dir, 'test_scores.csv')) as file:
        ...
        with open(os.path.join(trial_dir, 'args.json')) as file:
            trial_args = json.load(file)
        ...
        if 'linked_hidden_size' in param_keys:
            ...
            if 'hidden_size' not in param_keys:
                if getattr(hyperopt_args,
                           'hidden_size') != trial_args['hidden_size']:
                    raise ValueError(
            ...
            if 'ffn_hidden_size' not in param_keys:
                if getattr(hyperopt_args,
                           'ffn_hidden_size') != trial_args['ffn_hidden_size']:
                    raise ValueError(
...
def save_config(config_path: str, hyperparams_dict: dict,
                max_lr: float) -> None:
    """
    ...
    for key in hyperparams_dict:
        if key == "linked_hidden_size":
            ...
            if "max_lr" not in hyperparams_dict:
                ...
                save_dict["init_lr"] = hyperparams_dict[key] * \
                    hyperparams_dict["max_lr"]
        elif key == "final_lr_ratio":
            if "max_lr" not in hyperparams_dict:
                save_dict["final_lr"] = hyperparams_dict[key] * max_lr
            else:
                save_dict["final_lr"] = hyperparams_dict[key] * \
                    hyperparams_dict["max_lr"]
        else:
