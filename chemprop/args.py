...
def get_checkpoint_paths(checkpoint_path: Optional[str] = None,
    ...
    """
    if sum(var is not None for var in [
           checkpoint_dir, checkpoint_path, checkpoint_paths]) > 1:
        raise ValueError(
...
class TrainArgs(CommonArgs):
    ...
    """
    reaction_mode: Literal['reac_prod',
                           'reac_diff',
                           'prod_diff',
                           'reac_prod_balance',
                           'reac_diff_balance',
                           'prod_diff_balance'] = 'reac_diff'
    """
    ...
    def minimize_score(self) -> bool:
        ...
        return self.metric in {'rmse', 'mae', 'mse', 'cross_entropy', 'binary_cross_entropy',
                               'sid', 'wasserstein', 'bounded_mse', 'bounded_mae', 'bounded_rmse'}
    ...
    def process_args(self) -> None:
        ...
        # Prevents the temporary directory from being deleted upon function
        # return
        ...
        # Check whether the number of input columns is two for the
        # reaction_solvent mode
        ...
        if self.checkpoint_paths is not None and len(
        ...
        for metric in self.metrics:
            if not any([(self.dataset_type == 'classification' and metric in ['auc', 'prc-auc', 'accuracy', 'binary_cross_entropy', 'f1', 'mcc']),
                        (self.dataset_type == 'regression' and metric in [
                         'rmse', 'mae', 'mse', 'r2', 'bounded_rmse', 'bounded_mae', 'bounded_mse']),
        ...
        if not (self.split_type == 'predetermined') == (
                self.folds_file is not None) == (self.test_fold_index is not None):
            raise ValueError(
        ...
        # Validate features are provided for separate validation or test set
        # for each of the kinds of additional features
...
class PredictArgs(CommonArgs):
    ...
    calibration_method: Literal['zscaling',
                                'tscaling',
                                'zelikman_interval',
                                'mve_weighting',
                                'platt',
    ...
    def process_args(self) -> None:
        ...
        if self.uncertainty_method == 'dropout' and version.parse(
                torch.__version__) < version.parse('1.9.0'):
            raise ValueError(
        ...
        # Validate that features provided for the prediction test set are also
        # provided for the calibration set
