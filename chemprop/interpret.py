""" Interpreter for Chemprop models. """
import math
...
class ChempropModel:
    ...
    def __init__(self, args: InterpretArgs) -> None:
        ...
        # If features were used during training, they must be used when
        # predicting
...
class MCTSNode:
    ...
    def __init__(self, smiles: str,
                 atoms: List[int], W: float = 0, N: int = 0, P: float = 0) -> None:
        """
...
def find_clusters(
        mol: Chem.Mol) -> Tuple[List[Tuple[int, ...]], List[List[int]]]:
    """
...
def __extract_subgraph(
        mol: Chem.Mol, selected_atoms: Set[int]) -> Tuple[Chem.Mol, List[int]]:
    """
    ...
    for idx in selected_atoms:
        ...
        bad_neis = [y for y in atom.GetNeighbors() if y.GetIdx()
    ...
    for atom_idx in roots:
        ...
        aroma_bonds = [bond for bond in atom.GetBonds(
    ...
    remove_atoms = [atom.GetIdx() for atom in new_mol.GetAtoms()
...
def extract_subgraph(
        smiles: str, selected_atoms: Set[int]) -> Tuple[str, List[int]]:
    """
...
def mcts_rollout(node: MCTSNode,
    ...
    if len(node.children) == 0:
        ...
        for i in cur_cls:
            leaf_atoms = [
                a for a in clusters[i] if len(
                    atom_cls[a] & cur_cls) == 1]
...
def mcts(smiles: str,
    ...
    rationales = [node for _, node in state_map.items() if len(
        node.atoms) <= max_atoms and node.P >= prop_delta]
...
def interpret(args: InterpretArgs) -> None:
    ...
    all_smiles = get_smiles(
        path=args.data_path,
        smiles_columns=args.smiles_columns)
    header = get_header(path=args.data_path)

    property_name = header[args.property_id] if len(
        header) > args.property_id else 'score'
