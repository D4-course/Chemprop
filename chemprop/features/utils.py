...
def load_valid_atom_or_bond_features(
        path: str, smiles: List[str]) -> List[np.ndarray]:
    """
    Loads features saved in a variety of formats.
    ...
    * :code:'.sdf' containing all mol blocks with descriptors as entries
    ...
    if extension == '.npz':
        ...
        features_df = features_df.iloc[:, features_df.iloc[0, :].apply(
            lambda x: isinstance(x, str) and ',' in x).to_list()]
