...
class Featurization_parameters:
    ...
    def __init__(self) -> None:
        ...
        self.THREE_D_DISTANCE_BINS = list(
            range(
                0,
                self.THREE_D_DISTANCE_MAX + 1,
                self.THREE_D_DISTANCE_STEP))

        # len(choices) + 1 to include room for uncommon values; + 2 at end for
        # IsAromatic and mass
        self.ATOM_FDIM = sum(
...
def get_bond_fdim(atom_messages: bool = False,
    ...
    if PARAMS.REACTION_MODE:
        return (not overwrite_default_bond) * PARAMS.BOND_FDIM + is_reaction * PARAMS.EXTRA_BOND_FDIM + \
            (not atom_messages) * get_atom_fdim(
...
class MolGraph:
    ...
    * :code:`reaction_mode`:  Reaction mode to construct atom and bond feature vectors
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if type(mol) == str:
            if self.is_reaction:
                mol = (make_mol(mol.split(">")[0], self.is_explicit_h, self.is_adding_hs), make_mol(
        ...
        if not self.is_reaction:
            ...
            if atom_features_extra is not None:
                if overwrite_default_atom_features:
                    ...
                    self.f_atoms = [
                        f_atoms +
                        descs.tolist() for f_atoms,
                        descs in zip(
                            self.f_atoms,
                            atom_features_extra)]

            self.n_atoms = len(self.f_atoms)
            if atom_features_extra is not None and len(
                    atom_features_extra) != self.n_atoms:
            ...
            if bond_features_extra is not None and len(
                    bond_features_extra) != self.n_bonds / 2:
            ...
            if self.reaction_mode in ['reac_diff', 'prod_diff', 'reac_prod']:
                # Reactant: regular atom features for each atom in the
                # reactants, as well as zero features for atoms that are only
                # in the products (indices in pio)
                ...
                # regular features for atoms that are only in the products
                # (indices in pio)
                ...
                # Reactant: regular atom features for each atom in the
                # reactants, copy features from product side for atoms that are
                # only in the products (indices in pio)
                f_atoms_reac = [atom_features(atom) for atom in mol_reac.GetAtoms(
                ...
                # other atoms, regular features for atoms that are only in the
                # products (indices in pio)
                f_atoms_prod = [atom_features(mol_prod.GetAtomWithIdx(ri2pi[atom.GetIdx()])) if atom.GetIdx() not in rio else
                ...
                for a2 in range(a1 + 1, self.n_atoms):
                    if a1 >= n_atoms_reac and a2 >= n_atoms_reac:  # Both atoms only in product
                        ...
                        if a1 in ri2pi.keys() and a2 in ri2pi.keys():
                            bond_prod = mol_prod.GetBondBetweenAtoms(
                                ri2pi[a1], ri2pi[a2])  # Both atoms in both reactant and product
                        else:
                            if self.reaction_mode in [
                                    'reac_prod_balance', 'reac_diff_balance', 'prod_diff_balance']:
