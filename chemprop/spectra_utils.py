...
def normalize_spectra(spectra: List[List[float]], phase_features: List[List[float]] = None, phase_mask: List[List[float]]
                      = None, batch_size: int = 50, excluded_sub_value: float = None, threshold: float = None) -> List[List[float]]:
    """
    ...
    for i in trange(0, num_iters, iter_step):
        ...
        if phase_exclusion:
            batch_phase_mask = np.matmul(
                batch_phases, phase_mask).astype('bool')
            # mask shows True only if both components true
            batch_mask = ~(~batch_mask + ~batch_phase_mask)
...
def roundrobin_sid(spectra: np.ndarray,
    ...
    for i in range(len(spectra)):
        ...
        loss = ensemble_head * np.log(ensemble_head / ensemble_tail) + \
