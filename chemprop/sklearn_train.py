    ...
    if dataset_type == 'regression':
        ...
        if model_type == 'random_forest':
            ...
            if type(preds) == list:
                ...
                preds = [[preds[i][j, 1]
                          for i in range(num_tasks)] for j in range(num_preds)]
            else:
                # One task
...
def impute_sklearn(model: Union[RandomForestRegressor, RandomForestClassifier, SVR, SVC],
    ...
    for task_num in trange(num_tasks):
        impute_train_features = [
            ...
            targets in zip(
                train_data.features(),
                train_data.targets()) if targets[task_num] is None]
...
def run_sklearn(args: SklearnTrainArgs,
    ...
    # Need to have val set so that train and test sets are the same as when
    # doing MPN
