"""Utility functions for loading and saving models."""
...
def save_checkpoint(
    ...
    data_scaler = {
        "means": scaler.means,
        "stds": scaler.stds} if scaler is not None else None
...
def load_checkpoint(
    ...
    for loaded_param_name in loaded_state_dict.keys():
        ...
        if re.match(
                r"(encoder\.encoder\.)([Wc])", loaded_param_name) and not args.reaction_solvent:
...
def create_logger(name: str, save_dir: str = None,
...
def save_smiles_splits(
    ...
    all_split_indices = []
    for dataset, name in [(train_data, "train"),
...
def update_prediction_args(
    ...
    if missing_to_defaults:
        ...
        # then that argument must be included in the `override_defaults`
        # dictionary to force the legacy behavior.
        override_defaults = {
    ...
    # Same number of molecules must be used in training as in making
    # predictions
    ...
    # If atom-descriptors were used during training, they must be used when
    # predicting and vice-versa
    ...
    # If bond features were used during training, they must be used when
    # predicting and vice-versa
    ...
    # if atom or bond features were scaled, the same must be done during
    # prediction
