...
def hyperopt(args: HyperoptArgs) -> None:
    ...
    * :code:`ffn_num_layers`: The number of feed-forward layers after message passing is selected from {1, 2, 3}
    ...
    logger = create_logger(
        name=HYPEROPT_LOGGER_NAME,
        save_dir=args.log_dir,
        quiet=True)

    # Build search space
    logger.info(
        f"Creating search space using parameters {args.search_parameters}.")
    ...
    def objective(
            hyperparams: Dict[str, Union[int, float]], seed: int) -> Dict:
        ...
        if args.save_dir is not None:
            ...
            hyper_args.save_dir = os.path.join(
                hyper_args.save_dir, folder_name)
        ...
        if 'init_lr_ratio' in hyperparams:
            hyper_args.init_lr = hyperparams['max_lr'] * \
                hyperparams['init_lr_ratio']
        if 'final_lr_ratio' in hyperparams:
            hyper_args.final_lr = hyperparams['max_lr'] * \
                hyperparams['final_lr_ratio']

        # Cross validate
        mean_score, std_score = cross_validate(
            args=hyper_args, train_func=run_training)
    ...
    for i in range(args.num_iters):
        # run fmin and load trials in single steps to allow for parallel
        # operation
        trials = load_trials(
            dir_path=args.hyperopt_checkpoint_dir,
            previous_trials=manual_trials)
        ...
        # Set a unique random seed for each trial. Pass it into objective
        ...
        os.environ['HYPEROPT_FMIN_SEED'] = str(hyperopt_seed)
        ...
        fmin(
            ...
            algo=partial(
                tpe.suggest,
                n_startup_jobs=args.startup_random_iters),
            max_evals=len(trials) + 1,
            trials=trials,
        )

        # Create a trials object with only the last instance by merging the
        # last data with an empty trials object
    ...
    results = [
        result for result in results if not np.isnan(
            result['mean_score'])]
    best_result = min(
        results,
        key=lambda result: (
            1 if args.minimize_score else -
            1) *
        result['mean_score'])
