...
class TScalingCalibrator(UncertaintyCalibrator):
    ...
    def raise_argument_errors(self):
        ...
        if self.num_models == 1:
            raise ValueError(
                "T scaling is intended for use with ensemble models.")
...
class ZelikmanCalibrator(UncertaintyCalibrator):
        ...
        for i in range(self.num_tasks):
            ...
            if self.regression_calibrator_metric == "interval":
                interval_scaling = np.percentile(
                    task_preds, self.interval_percentile)
                self.scaling[i] = interval_scaling
            else:
    ...
    def nll(
        ...
        for i in range(self.num_tasks):
            ...
            # magnitude adjusted by stdev scale of the distribution and
            # symmetry assumption
...
class MVEWeightingCalibrator(UncertaintyCalibrator):
    ...
    def calibrate(self):
        ...
        self.var_weighting = np.zeros(
            [self.num_models, targets.shape[1]])  # shape(models, tasks)
        ...
        for i in range(targets.shape[1]):
            ...
            def objective(scaler_values: np.ndarray):
                scaler_values = np.reshape(
                    softmax(scaler_values), [-1, 1])  # (models, 1)
                scaled_vars = np.sum(
    ...
    def apply_calibration(self, uncal_predictor: UncertaintyPredictor):
        ...
        uncal_individual_vars = np.array(
            uncal_predictor.get_individual_vars())  # shape(models, data, tasks)
...
class PlattCalibrator(UncertaintyCalibrator):
    ...
    def calibrate(self):
        ...
        # If train class sizes are available, set Bayes corrected calibration
        # targets
    ...
    def apply_calibration(self, uncal_predictor: UncertaintyPredictor):
        uncal_preds = np.array(
            uncal_predictor.get_uncal_preds())  # shape(data, task)
        cal_preds = expit(
    ...
    def nll(self, preds: List[List[float]], unc: List[List[float]],
            targets: List[List[float]], mask: List[List[bool]]):
        targets = np.array(targets, dtype=float)
        mask = np.array(mask, dtype=bool)
        unc = np.array(unc)
        nll = []
        for i in range(targets.shape[1]):
            ...
            likelihood = task_unc * task_targets + \
...
class IsotonicMulticlassCalibrator(UncertaintyCalibrator):
    ...
    def calibrate(self):
        ...
        targets = np.array(
            self.calibration_data.targets(),
            dtype=float)  # shape(data, tasks)
        mask = np.array(self.calibration_data.mask(), dtype=bool)
        self.num_tasks = targets.shape[1]
    ...
    def apply_calibration(self, uncal_predictor: UncertaintyPredictor):
        ...
        for i in range(self.num_tasks):
            ...
            for j in range(self.num_classes):
                ...
                # shape (task, class, data)
                transpose_cal_preds[i].append(class_cal)
