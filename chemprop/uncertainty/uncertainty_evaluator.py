    ...
    def evaluate(
        ...
        if self.calibrator is not None:
            ...
            for j in range(targets.shape[1]):
                ...
                for i in range(1, 100):
                    bin_unc = task_unc / \
                        original_scaling[j] * bin_scaling[i][j]
        ...
        # trapezoid rule
        auce = np.sum(
            0.01 * np.abs(fractions -
                          np.expand_dims(np.arange(101) / 100, axis=0)),
            axis=1,
        )
        return auce.tolist()
    ...
    def evaluate(
        ...
        for i in range(targets.shape[1]):
            ...
            split_unc = np.array_split(
                task_unc, 100)  # shape(list100, data/100)
            ...
            for j in range(100):
                if self.calibrator is None:  # starts as a variance
                    ...
                    bin_var = t.var(
                        df=self.calibrator.num_models - 1, scale=bin_unc)
                    ...
                    if self.calibrator.regression_calibrator_metric == "interval":
                        ...
                        bin_unc = bin_unc / \
                            original_scaling[i] * stdev_scaling[i]
