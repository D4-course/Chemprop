...
class ChempropTests(TestCase):
    ...
    def create_raw_predict_args(dataset_type: str,
        ...
        return [
            ...
            '--test_path', os.path.join(TEST_DATA_DIR,
                                        f'{dataset_type}_test_smiles.csv'),
    ...
    def create_raw_interpret_args(dataset_type: str,
        ...
        return [
            ...
            '--data_path', os.path.join(TEST_DATA_DIR,
                                        f'{dataset_type}_test_smiles.csv'),
            '--checkpoint_dir', checkpoint_dir
        ] + (flags if flags is not None else [])
    ...
    @parameterized.expand([
        ...
        (
            ...
            2.14015989,
            ['--features_path',
             os.path.join(TEST_DATA_DIR, 'regression.npz'), '--no_features_scaling']
        ),
        (
    ...
    def test_train_single_task_regression(self,
        ...
        """Tests training a single-task regression model."""
    ...
    @parameterized.expand([
        ...
        (
            ...
            'auc',
            0.519689086,
            ['--features_generator', 'morgan', '--class_balance',
        ...
        (
            ...
            'auc',
            0.466828424,
            ['--features_path', os.path.join(TEST_DATA_DIR, 'classification.npz'),
             '--no_features_scaling', '--class_balance', '--split_sizes', '0.4', '0.3', '0.3']
        ),
        (
            'chemprop_mcc_metric',
            'chemprop',
            'mcc',
            0.014589067,
            ['--metric', 'mcc', '--data_path',
             os.path.join(TEST_DATA_DIR, 'classification_common.csv'), '--class_balance']
        ),
        (
            'chemprop_f1_metric',
            'chemprop',
    ...
    def test_train_multi_task_classification(self,
                                             ...
                                             expected_score: float,
                                             train_flags: List[str] = None):
        """Tests training a multi-task classification model."""
        with TemporaryDirectory() as save_dir:
            # Train
            self.train(
    ...
    @parameterized.expand([
        ...
        (
            ...
            ['--features_path',
             os.path.join(TEST_DATA_DIR, 'regression.npz'), '--no_features_scaling'],
            ['--features_path',
             os.path.join(TEST_DATA_DIR, 'regression_test.npz'), '--no_features_scaling']
        )
    ])
    def test_predict_single_task_regression(self,
        ...
        """Tests predicting with a single-task regression model."""
        with TemporaryDirectory() as save_dir:
            # Train
            dataset_type = 'regression'
            self.train(
    ...
    @parameterized.expand([
        ...
        (
            ...
            ['--features_path', os.path.join(TEST_DATA_DIR, 'classification.npz'),
             '--no_features_scaling', '--class_balance', '--split_sizes', '0.4', '0.3', '0.3'],
            ['--features_path',
             os.path.join(TEST_DATA_DIR, 'classification_test.npz'), '--no_features_scaling']
        )
    ])
    def test_predict_multi_task_classification(self,
                                               ...
                                               train_flags: List[str] = None,
                                               predict_flags: List[str] = None):
        """Tests predicting with a multi-task classification model."""
        with TemporaryDirectory() as save_dir:
            # Train
            dataset_type = 'classification'
            self.train(
    ...
    def test_chemprop_hyperopt(self):
        """Tests hyperparameter optimization with Chemprop."""
        with TemporaryDirectory() as save_dir:
            # Train
            ...
            parameters = {'depth': (2, 6), 'hidden_size': (300, 2400), 'ffn_hidden_size': (
                300, 2400), 'ffn_num_layers': (1, 3), 'dropout': (0.0, 0.4)}
    ...
    def test_interpret_single_task_regression(self,
        ...
        """Tests interpreting a single-task regression model."""
        with TemporaryDirectory() as save_dir:
            # Train
            dataset_type = 'regression'
            self.train(
    ...
    @parameterized.expand([
        (
            ...
            [
                ...
                '--features_path', os.path.join(TEST_DATA_DIR,
                                                'spectra_features.csv'),
        ...
        (
            ...
            [
                '--data_path', os.path.join(TEST_DATA_DIR,
                                            'spectra_exclusions.csv'),
                '--phase_features_path', os.path.join(
                    TEST_DATA_DIR, 'spectra_features.csv'),
                '--spectra_phase_mask_path', os.path.join(
                    TEST_DATA_DIR, 'spectra_mask.csv'),
                '--split_type', 'random_with_repeated_smiles'
            ]
        ),
    ])
    def test_train_spectra(self,
                           ...
                           expected_score: float,
                           train_flags: List[str] = None):
        """Tests training a spectra model."""
        with TemporaryDirectory() as save_dir:
            # Train
            metric = 'sid'
            self.train(
    ...
    def test_predict_spectra(self,
        ...
        with TemporaryDirectory() as save_dir:
            ...
            sid = evaluate_predictions(preds=pred, targets=true, num_tasks=len(
                true[0]), metrics=['sid'], dataset_type='spectra')['sid'][0]
            self.assertAlmostEqual(sid, expected_score,
                                   delta=DELTA * expected_score)
    ...
    @parameterized.expand([
        ...
        (
            ...
            'chemprop',
            2.18239804,
            ['--reaction', '--data_path', os.path.join(
                TEST_DATA_DIR, 'reaction_regression.csv'), '--split_type', 'scaffold_balanced']
        ),
        (
            'chemprop_morgan_features_generator',
            'chemprop',
            3.122113679,
            ['--reaction', '--data_path', os.path.join(
                TEST_DATA_DIR, 'reaction_regression.csv'), '--features_generator', 'morgan']
        ),
        (
            ...
            2.34536046,
            ['--reaction', '--data_path',
             os.path.join(TEST_DATA_DIR, 'reaction_regression.csv'), '--explicit_h']
        )
    ])
    def test_train_single_task_regression_reaction(self,
                                                   ...
                                                   expected_score: float,
                                                   train_flags: List[str] = None):
        """Tests training a single-task regression model on reaction data."""
        with TemporaryDirectory() as save_dir:
            # Train
            metric = 'rmse'
            self.train(
    ...
    @parameterized.expand([
        (
            ...
            'auc',
            0.6644781145,
            ['--number_of_molecules', '2', '--data_path',
    ...
    def test_single_task_multimolecule_classification(self,
                                                      ...
                                                      expected_score: float,
                                                      train_flags: List[str] = None):
        """Tests training a single-task classification model on multimolecule data."""
        with TemporaryDirectory() as save_dir:
            # Train
            self.train(
                dataset_type='classification',
    ...
    def test_single_task_fingerprint(self,
        ...
        """Tests training a single-task regression model on fingerprint data."""
        with TemporaryDirectory() as save_dir:
            # Train
            dataset_type = 'classification'
            self.train(
            ...
            fingerprints = pd.read_csv(
                fingerprint_path).drop(["smiles"], axis=1)
            self.assertAlmostEqual(
    ...
    @parameterized.expand([
        ...
        (
            ...
            False,
            ['--number_of_molecules', '2', '--data_path',
             os.path.join(TEST_DATA_DIR, 'classification_multimolecule.csv'), '--mpn_shared']
        ),
        (
            'chemprop',
            'chemprop',
    ...
    def test_multimolecule_fingerprint_with_single_input(self,
        ...
        with TemporaryDirectory() as save_dir:
            ...
            if exception_thrown:
                ...
                test_input = pd.read_csv(os.path.join(
                    TEST_DATA_DIR, f'{dataset_type}_test_smiles.csv'))

                self.assertEqual(list(fingerprints['smiles'].values), list(
                    test_input['smiles'].values))
    ...
    @parameterized.expand([(
        ...
        ['--reaction_solvent', '--number_of_molecules', '2',
         '--data_path', os.path.join(
             TEST_DATA_DIR, 'reaction_solvent_regression.csv'), '--hidden_size', '500',
    ...
    def test_train_single_task_regression_reaction_solvent(self,
                                                           ...
                                                           expected_score: float,
                                                           train_flags: List[str] = None):
        """Tests training a single-task regression model on reaction solvent data."""
        with TemporaryDirectory() as save_dir:
            # Train
            metric = 'rmse'
            self.train(
    ...
    def test_uncertainty_regression(
        ...
        """Tests uncertainty methods on regression data."""
        with TemporaryDirectory() as save_dir:
            ...
            test_path = os.path.join(
                TEST_DATA_DIR, 'regression_multitask_gaps.csv')
            ...
            self.assertAlmostEqual(
                evaluation_scores_data['homo'][0], expected_score, delta=np.abs(expected_score * DELTA))
    ...
    def test_uncertainty_class(
        ...
        """Tests uncertainty methods on classification data."""
        with TemporaryDirectory() as save_dir:
            test_path = os.path.join(
                TEST_DATA_DIR, 'classification_multimolecule.csv')
