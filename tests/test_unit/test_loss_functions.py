...
@pytest.mark.parametrize(
    ...
    [
        (torch.tensor([[2, 2]], dtype=float),
         torch.ones([1, 1], dtype=float), 0, [[0.6]]),
        (torch.tensor([[2, 2]], dtype=float), torch.ones(
            [1, 1], dtype=float), 0.2, [[0.63862943]]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (torch.tensor([[2, 2, 2, 2]], dtype=float),
         torch.ones([1, 1], dtype=float), 0, [[1.56893861]]),
        (torch.tensor([[2, 2, 2, 2]], dtype=float), torch.ones(
            [1, 1], dtype=float), 0.2, [[2.768938541]]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            torch.tensor([[0.2, 0.7, 0.1], [0.8, 0.1, 0.1],
                         [0.2, 0.3, 0.5]], dtype=float),
        ...
        (
            torch.tensor([[0.8, 0.1, 0.1], [0.7, 0.2, 0.1],
...
def test_multiclass_mcc(predictions, targets,
                        data_weights, mask, expected_loss):
    """
