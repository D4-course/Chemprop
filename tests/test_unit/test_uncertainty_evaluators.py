...
def test_build_unsupported_metrics(metric, dataset_type):
    ...
    with pytest.raises(NotImplementedError):
        build_uncertainty_evaluator(
            metric, None, None, dataset_type, None, None)


@pytest.mark.parametrize("targets,preds,uncs,mask,likelihood",
                         [([[0]], [[0]], [[1]], [[1]], [0.3989])])
def test_nll_regression(nll_regression_evaluator,
                        targets, preds, uncs, mask, likelihood):
    """
    Tests the result of the NLL regression UncertaintyEvaluator.
    """
    nll_calc = np.array(
        nll_regression_evaluator.evaluate(
            targets, preds, uncs, mask))
    likelihood_calc = np.exp(-1 * nll_calc)

    np.testing.assert_array_almost_equal(
        likelihood, likelihood_calc, decimal=4)
...
def test_nll_classificiation(
    ...
    """
    nll_calc = np.array(
        nll_classification_evaluator.evaluate(
            targets, preds, uncs, mask))
    likelihood_calc = np.exp(-1 * nll_calc)
...
@pytest.mark.parametrize(
    ...
    [
        (np.zeros((100, 1)), np.zeros((100, 1)), np.ones(
            (100, 1)), np.full((100, 1), True, dtype=bool), [0.495]),
        (np.full((100, 1), 100), np.zeros((100, 1)), np.ones(
            (100, 1)), np.full((100, 1), True, dtype=bool), [0.495]),
    ],
)
def test_miscal_regression(miscal_regression_evaluator,
                           targets, preds, uncs, mask, area_exp):
    """
