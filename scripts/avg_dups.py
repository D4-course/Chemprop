...
class Args(Tap):
    """Arguments for averaging duplicate data points in a dataset."""
    data_path: str  # Path to data CSV file
    # Name of the columns containing SMILES strings. By default, uses the
    # first column.
    smiles_columns: List[str] = None
    # Name of the columns containing target values. By default, uses all
    # columns except the SMILES column.
...
def average_duplicates(args: Args):
    ...
    for smiles in smiles_in_order:
        ...
        stds.append([np.std(task_targets) if len(task_targets) >
                    0 else 0.0 for task_targets in targets_by_task])
        means = [np.mean(task_targets) if len(task_targets) >
                 0 else None for task_targets in targets_by_task]
