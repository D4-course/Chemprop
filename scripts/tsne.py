...
def compare_datasets_tsne(args: Args):
    ...
    morgans = [morgan_generator(smile)
    ...
    plt.title(
        't-SNE using Morgan fingerprint with Jaccard similarity',
        fontsize=2 * fontsize)
    ax = fig.gca()
    handles = []
    ...
    if args.cluster:
        plt.scatter(X[:, 0], X[:, 1], s=150 *
                    np.mean(args.sizes), c=colors, cmap='nipy_spectral')
    else:
        for slc, color, label, size in zip(
                slices, args.colors, labels, args.sizes):
            if args.plot_molecules:
                ...
                # Plots points
                plt.scatter(X[slc, 0], X[slc, 1], s=150 *
                            size, color=color, label=label)
