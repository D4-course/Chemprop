"""calculate class balance for each task in a dataset"""
import os
import sys
...
class Args(Tap):
    ...
    split_type: Literal['random', 'scaffold'] = 'scaffold'


def class_balance(args: Args):
    """Prints class balance for each task in a dataset."""
    # Update args
    ...
    for split_name in ['train', 'val', 'test']:
        ...
        mean_class_sizes, std_class_sizes = np.mean(
            all_class_sizes[split_name], axis=0), np.std(
