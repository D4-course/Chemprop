...
class Args(Tap):
    ...
    smiles_columns_1: List[str] = None
    # Name of the column containing SMILES strings for the second data. By
    # default, uses the first column.
    smiles_columns_2: List[str] = None
    # Path to save intersection at; labeled with data_path 1 header
    save_intersection_path: str = None
    # Path to save molecules in dataset 1 that are not in dataset 2; labeled
    # with data_path 1 header
    save_difference_path: str = None
...
def overlap(args: Args):
    ...
    smiles_1 = get_smiles(
        ...
        smiles_columns=args.smiles_columns_1)
    smiles_2 = get_smiles(
        path=args.data_path_2,
        smiles_columns=args.smiles_columns_2)
