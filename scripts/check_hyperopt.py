...
class Args(Tap):
    """Arguments """
    # Paths to directory containing hyperopt config.json files in directories
    # labelled by fold number (0, 1, ...)
    ckpts_dirs: List[str]
    ...
    for ckpts_dir in ckpts_dirs:
        ...
        for root, _, files in os.walk(ckpts_dir):
            ...
            fnames += [os.path.join(root, fname)
                       for fname in files if fname == 'config.json']

        # Print out complete and incomplete
        complete = {int(os.path.basename(os.path.dirname(fname)))
                    for fname in fnames}
        ...
        print(
            f'incomplete = {" ".join(str(fold) for fold in sorted(incomplete))}')
        print()
