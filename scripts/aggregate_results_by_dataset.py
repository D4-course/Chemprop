...
class Args(Tap):
    """Arguments for aggregating results by dataset."""
    dataset: str  # Dataset to collect results for
    ckpt_dir: str  # Path to directory containing all checkpoints which will be walked to find
    # checkpoints for the dataset Split type, either "random" or "scaffold"
    split_type: Literal['random', 'scaffold']


def aggregate_results_by_dataset(dataset: str, ckpt_dir: str, split_type: str):
    """ Aggregates results for a given dataset and split type. """
    ...
    for experiment in EXPERIMENTS:
        ...
        for root, _, files in os.walk(exp_dir):
            paths += [os.path.join(root, fname)
                      for fname in files if fname == 'verbose.log']
    ...
    for i in range(max_num_folds):
        for experiment in EXPERIMENTS:
            print(
                results[experiment][i] if i < len(
                    results[experiment]) else '',
                end='\t')
        print()
