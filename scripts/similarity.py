...
class Args(Tap):
    """Arguments for computing scaffold similarity."""
    data_path_1: str  # Path to first data CSV file
    data_path_2: str  # Path to second data CSV file
    # Name of the column containing SMILES strings for the first data. By
    # default, uses the first column.
    smiles_column_1: str = None
    # Name of the column containing SMILES strings for the second data. By
    # default, uses the first column.
    smiles_column_2: str = None
    # Similarity measure to use to compare the two datasets
    similarity_measure: Literal['scaffold', 'morgan']
    radius: int = 3  # Radius of Morgan fingerprint
    # Rate at which to sample pairs of molecules for Morgan similarity (to
    # reduce time)
...
def scaffold_similarity(smiles_1: List[str], smiles_2: List[str]):
    ...
    smiles_to_scaffold = {
        smiles: scaffold for scaffold,
        smiles_set in scaffold_to_smiles_1.items() for smiles in smiles_set}
    smiles_to_scaffold.update({smiles: scaffold for scaffold,
                               smiles_set in scaffold_to_smiles_2.items() for smiles in smiles_set})
    ...
    # smiles_intersection is smiles with a scaffold that appears in both
    # datasets
    smiles_intersection = {
        smiles for smiles in all_smiles if smiles_to_scaffold[smiles] in scaffolds_intersection}
...
def morgan_similarity(
    ...
    if sample_rate < 1.0:
        ...
        sample_smiles_1 = np.random.choice(
            smiles_1, size=sample_size, replace=True)
        sample_smiles_2 = np.random.choice(
            smiles_2, size=sample_size, replace=True)
    else:
    ...
    for smile_1, smile_2 in tqdm(
        ...
        fp_1, fp_2 = AllChem.GetMorganFingerprint(
            mol_1, radius), AllChem.GetMorganFingerprint(
            mol_2, radius)
