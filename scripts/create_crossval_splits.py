...
class Args(Tap):
    ...
    split_type: Literal['random', 'scaffold', 'time_window']
    ...
    # X:1:1 train:val:test for time split sliding window
    time_folds_per_train_set: int = 3
    # columns in CSV dataset file containing SMILES
    smiles_columns: List[str] = None
    # index of the molecule to use for splitting in muli-molecule data
...
def split_indices(all_indices: List[int],
    ...
    if scaffold:
        ...
        for i in range(num_folds):
            begin, end = int(
                i * num_data / num_folds), int((i + 1) * num_data / num_folds)
...
def create_time_splits(args: Args):
    ...
    # this will dump a very different format of indices, with all in one file;
    # TODO modify as convenient later.
        ...
        for i in range(len(fold_indices[split_type])):
            ...
            with open(os.path.join(args.save_dir, split_type, 'fold_' + str(i), '0', 'split_indices.pckl'), 'wb') as wf:
                train = np.concatenate(
                    [fold_indices[split_type][i][j] for j in range(args.time_folds_per_train_set)])
                # train = []
...
def create_crossval_splits(args: Args):
    ...
    if args.split_type == 'random':
        ...
        fold_indices = split_indices(
            all_indices, args.num_folds, scaffold=False)
