...
def load_preds_and_targets(preds_dir: str,
                           ...
                           split_type: str) -> Tuple[Optional[np.ndarray],
                                                     Optional[np.ndarray]]:
    """ Loads predictions and targets for a given experiment and dataset. """
    ...
    all_preds, all_targets = np.concatenate(
...
def compute_values(dataset: str,
    ...
    """ Computes the values for a given dataset. """
...
def wilcoxon_significance(preds_dir: str, split_type: str):
    """ Performs Wilcoxon signed-rank tests for significance. """
    print('dataset\t' +
          '\t'.join([f'{exp_1} vs {exp_2}' for exp_1, exp_2 in COMPARISONS]))
    ...
    for dataset in DATASETS:
        ...
        for experiment in EXPERIMENTS:
            ...
            preds, targets = load_preds_and_targets(
                preds_dir, experiment, dataset, split_type)  # num_molecules x num_targets
            ...
            if dataset_type == 'regression':
                ...
                preds, targets = np.array_split(
                    preds, 30), np.array_split(
                    targets, 30)
        ...
        for experiment_1, experiment_2 in COMPARISONS:
            if 'compare_lsc_scaffold' in [
                    experiment_1, experiment_2] and split_type != 'scaffold':
                continue
            ...
            values_1, values_2 = zip(
                *[(v_1, v_2) for v_1, v_2 in zip(values_1, values_2) if not (np.isnan(v_1) or np.isnan(v_2))])
            ...
            print(
                wilcoxon(
                    values_1,
                    values_2,
                    alternative='less' if dataset_type == 'regression' else 'greater').pvalue,
                end='\t')
        print()
